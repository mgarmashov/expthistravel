{{--this file us updated with script that allows to set items in order as they are chosen
        ...
        @if (isset($field['model']))
            ...
            foreach ($options as $option) {
                if( (old(square_brackets_to_dots($field["name"])) && in_array($option->getKey(), old($field["name"]))) || (is_null(old(square_brackets_to_dots($field["name"]))) && isset($field['value']) && in_array($option->getKey(), $field['value']->pluck($option->getKeyName(), $option->getKeyName())->toArray()))) {
                    $selectedElements .= '<option value="'.$option->getKey().'" selected>'.$option->{$field['attribute']}.'</option>';
                } else {
                    $unselectedElements .= '<option value="'.$option->getKey().'">'.$option->{$field['attribute']}.'</option>';
                }
            }
            @endphp
            {!! $selectedElements !!}
